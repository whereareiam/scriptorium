...
export function createGitCliStageRepository(projectRoot: string): StageRepository {
  ...
  return {
    ...
    async listFiles(refName, localProjectRoot) {
      ...
      return filterBundledContractPaths(output.split("\n"), realProjectRoot, realRepoRoot);
    },
...
export async function syncGitCliRepository(options: {
  repoDir: string;
  repoUrl: string;
...
async function createGitEnvironment(repoDir: string, options: {
  authToken?: string;
  authUsername?: string;
}) {
