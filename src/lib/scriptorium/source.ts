...
import { getLocalProjectRoot } from "./runtime-config";
...
export const getStagedSite = cache(async (projectRoot = getLocalProjectRoot()) => {
  if (process.env.SCRIPTORIUM_REPO_URL) {
    await ensureRuntimeSnapshot();
    return readStageManifest(projectRoot, getRuntimePaths().stagedDir);
  }
...
export const getProjectConfig = cache(async (projectRoot = getLocalProjectRoot()) => {
  const manifest = await getStagedSite(projectRoot);
  return manifest.project;
});

export const getPublishedRefs = cache(async (projectRoot = getLocalProjectRoot()) => {
  const manifest = await getStagedSite(projectRoot);
...
export const getSource = cache(async (projectRoot = getLocalProjectRoot()) => {
  const manifest = await getStagedSite(projectRoot);
...
export async function readCurrentAsset(assetSegments: string[], projectRoot = getLocalProjectRoot()) {
