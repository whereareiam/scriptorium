...
async function createFixtureRepo() {
  ...
  await writeJson(path.join(projectRoot, "scriptorium.project.json"), {
    ...
    refs: {
      dev: {
        label: "Development",
        description: "Default branch docs"
      }
    },
    refRules: {
      defaultBranch: "dev",
    ...
    links: [{ label: "Repository", url: "https://example.invalid/repo" }]
  });
...
describe("stageProjectRefs", () => {
  it("exports only allowed refs and only bundled contract files", async () => {
    ...
    const stagedDraftPath = path.join(projectRoot, ".scriptorium", "staged", "refs", "dev", "draft.md");
    await expect(readFile(stagedDraftPath, "utf8")).rejects.toThrow();
